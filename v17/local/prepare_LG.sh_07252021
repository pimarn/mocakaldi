...
utils/prepare_lang.sh $dict_out_dir '!SIL' $dict_out_dir/lang_tmp $lang_out_dir

[ -f $lm.gz ] && rm $lm.gz
gzip -k $lm
#../../babel/s5/local/arpa2G.sh $lm.gz $lang_out_dir $lang_out_dir
#/kaldi/egs/babel/s5/local/arpa2G.sh $lm.gz $lang_out_dir $lang_out_dir

lmfile=$lm.gz
gunzip -c $lmfile | \
	    arpa2fst --disambig-symbol=#0 \
	    		 --max_arpa_warnings=-1 \
	                 --read-symbol-table=$lang_out_dir/words.txt - $lang_out_dir/G.fst || exit 1
fstisstochastic $lang_out_dir/G.fst || true
